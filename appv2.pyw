...
NOMBRE_ARCHIVO = "Registro_JUNIO25.xlsx"
...
def cargar_excel():
    ...
    if not os.path.exists(NOMBRE_ARCHIVO):
        messagebox.showerror(
            "Error",
            f"El archivo {NOMBRE_ARCHIVO} no existe. Por favor, crea el archivo antes de continuar. El archivo debe contener una hoja llamada 'Asesores' con los datos de los asesores (Nombre, Matricula, Carrera, Programa).",
        )
        return
    return openpyxl.load_workbook(NOMBRE_ARCHIVO)


# Nombre, Matrícula, Hora de Entrada, Hora de Salida, Horas Recuperadas, Fecha de falta, Programa, Carrera
def inicializar_excel():
    ...
    if fecha_hoy not in wb.sheetnames:
        ...
        try:
            ...
            messagebox.showerror(
                "Error",
                f"Acceso denegado, asegurate de que el archivo {NOMBRE_ARCHIVO} no este abierto",
            )
            return
...
def registrar_entrada(event):
    ...
    ws_dia.append([asesor[0], asesor[1], hora_entrada, "", "", "", asesor[2], asesor[3]])
    ...
    messagebox.showinfo(
        "Éxito", f"Entrada registrada para {asesor[0]} ({asesor[2]} - Programa de {asesor[3]}) a las {hora_entrada}"
    )
    ...
    for row in ws_dia.iter_rows(min_row=2):
        if str(row[1].value).strip() == matricula and not row[3].value:
            ...
            try:
                ...
                mins = (diff % 3600) // 60
                segs = diff % 60
                messagebox.showinfo("Éxito", f"Salida registrada con éxito para {row[0].value} ({row[6].value} - Programa de {row[7].value}), realizó {int(horas):02}:{int(mins):02}:{int(segs):02} horas")
                borrar_inputs()
                return
            except PermissionError:
                messagebox.showerror(
...
# Interfaz gráfica
root = tk.Tk()
root.title("Gestión de Asesores y Talentos")
