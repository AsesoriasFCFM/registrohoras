...
def inicializar_excel():
    ...
    if fecha_hoy not in wb.sheetnames:
        ...
        ws_dia.append(["Nombre", "Matrícula", "Hora de Entrada", "Hora de Salida", "Horas Recuperadas", "Fecha de Falta"])
        try:
            wb.save("Asesores.xlsx")
        except PermissionError:
            messagebox.showerror("Error", f"Acceso denegado, asegurate de que el archivo Asesores.xlsx no este abierto")
            return
...
def registrar_entrada(event):
    ...
    for row in ws_dia.iter_rows(min_row=2):
        if str(row[1].value).strip() == matricula and not row[3].value:
            messagebox.showwarning("Aviso", "Existe una entrada para este asesor sin registro de salida. Favor de registrar la salida primero.")
            return
    ...
    messagebox.showinfo("Éxito", f"Entrada registrada para {asesor[0]} a las {hora_entrada}")
...
def registrar_salida(event):
    ...
    for row in ws_dia.iter_rows(min_row=2):
        if str(row[1].value).strip() == matricula and not row[3].value:
            ...
            try:
                wb.save("Asesores.xlsx")
                messagebox.showinfo("Éxito", f"Salida registrada con éxito")
                return
            except PermissionError:
                messagebox.showerror("Error", f"Acceso denegado, asegurate de que el archivo Asesores.xlsx no este abierto")
                return

    messagebox.showerror("Error", f"No se encontró una entrada pendiente para esta matrícula")
...
def registrar_recuperacion(event):
    ...
    messagebox.showerror("Error", f"No se pudo encontrar un registro abierto del asesor para las horas de recuperacion. Favor de registrar entrada sin registrar salida primero")
...
tk.Label(root, text="Ingrese Matrícula:", font=("Arial", 12, "bold"), bg="#f0f0f0").pack(pady=5)
entrada_matricula = tk.Entry(root, font=("Arial", 12))
entrada_matricula.pack(pady=5)

btn_entrada = tk.Button(root, text="Registrar Entrada (Enter)", font=("Arial", 12), bg="#4CAF50", fg="white")
btn_entrada.bind('<Button-1>', registrar_entrada)
btn_entrada.pack(pady=5, fill=tk.X)

btn_salida = tk.Button(root, text="Registrar Salida (RControl)", font=("Arial", 12), bg="#FF9800", fg="white")
btn_salida.bind('<Button-1>', registrar_salida)
btn_salida.pack(pady=5, fill=tk.X)

tk.Label(root, text="Ingrese Horas a Recuperar:", font=("Arial", 12, "bold"), bg="#f0f0f0").pack(pady=5)
entrada_horas = tk.Entry(root, font=("Arial", 12))
entrada_horas.pack(pady=5)

tk.Label(root, text="Ingrese Fecha de Falta:", font=("Arial", 12, "bold"), bg="#f0f0f0").pack(pady=5)
entrada_fecha_falta = tk.Entry(root, font=("Arial", 12))
entrada_fecha_falta.pack(pady=5)

btn_recuperar = tk.Button(root, text="Registrar Recuperación (LAlt)", font=("Arial", 12), bg="#2196F3", fg="white")
